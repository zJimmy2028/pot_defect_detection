...
class PipeDataset(Dataset):
	...
	def __getitem__(self, item):
		...
		LabelImgPath = os.path.join(SampleFolderPath, 'label_1.png')
		# LabelImg = Image.open(LabelImgPath)
		LabelImg = cv2.imread(LabelImgPath, cv2.IMREAD_GRAYSCALE)
		LabelImg = cv2.dilate(LabelImg, np.ones((7, 7), np.uint8))
		LabelImg = Image.fromarray(LabelImg)
		TMImgPath = os.path.join(SampleFolderPath, 'img02.png')
