...
class Net(nn.Module):
	def __init__(self, InputChannels=1, OutputChannels=1, InitFeatures=64, WithActivateLast=True, ActivateFunLast=None):
		...
		self.MP2 = nn.MaxPool2d(kernel_size=SmlKernel, stride=SmlKernel)
		self.EB3 = self.Block(IF * 2 * DoubleFeatures, IF * 4 * DoubleFeatures, Name="EB3")
		self.MP3 = nn.MaxPool2d(kernel_size=SmlKernel, stride=SmlKernel)
		self.EB4 = self.Block(IF * 4 * DoubleFeatures, IF * 8 * DoubleFeatures, Name="EB4")
		self.MP4 = nn.MaxPool2d(kernel_size=SmlKernel, stride=SmlKernel)

		self.EB5orDB5 = self.Block(IF * 8 * DoubleFeatures, IF * 16 * DoubleFeatures, Name="EB5orDB5")

		self.CT4 = nn.ConvTranspose2d(IF * 16 * DoubleFeatures, IF * 8 * DoubleFeatures, kernel_size=SmlKernel, stride=SmlKernel)
		self.DB4 = self.Block((IF * 8 * DoubleFeatures) * 2, IF * 8 * DoubleFeatures, Name="DB4")
		self.CT3 = nn.ConvTranspose2d(IF * 8 * DoubleFeatures, IF * 4 * DoubleFeatures, kernel_size=SmlKernel, stride=SmlKernel)
		self.DB3 = self.Block((IF * 4 * DoubleFeatures) * 2, IF * 4 * DoubleFeatures, Name="DB3")
		self.CT2 = nn.ConvTranspose2d(IF * 4 * DoubleFeatures, IF * 2 * DoubleFeatures, kernel_size=SmlKernel, stride=SmlKernel)
	...
	def Block(self, InputChannels, OutputChannels, Name):
		return nn.Sequential(OrderedDict([
			(Name + "_Conv1", nn.Conv2d(in_channels=InputChannels, out_channels=OutputChannels, kernel_size=5,dilation =2, padding=4, bias=False)),
			(Name + "_Norm1", nn.BatchNorm2d(num_features=OutputChannels)),
			(Name + "_Relu1", nn.ReLU(inplace=True)),
			# (Name + "_Drop1", nn.Dropout2d(0.5)),
			(Name + "_Conv2", nn.Conv2d(in_channels=OutputChannels, out_channels=OutputChannels, kernel_size=5, dilation =2, padding=4, bias=False)),
...
if __name__ == '__main__':
	...
	Input = torch.randn((1, 1, 4096, 4096)).cuda()
	Output = Model(Input)
	print(Output.shape)
	...
	# Input = torch.randn((3, 1, 1024, 1280)).cuda()
